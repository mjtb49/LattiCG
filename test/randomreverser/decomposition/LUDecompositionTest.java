...
public class LUDecompositionTest {
	...
	public void testLU1() {
		...
		assertTrue(m.equals(result.getP().multiply(result.getL().multiply(result.getU())), 0.0001F));
	}
	...
	public void testDet1() {
		Matrix m = Matrix.identityMatrix(4);
		LUResult result = LUDecomposition.decompose(m);
		assertEquals(1.0D, result.getDet(), 0.0D);
	}
	...
	public void testDet2() {
		Matrix m = Matrix.fromString("{{1, 5}, {2, -3}}");
		LUResult result = LUDecomposition.decompose(m);
		assertEquals(-13.0D, result.getDet(), 0.0D);
	}
	...
	public void testDet3() {
		Matrix m = Matrix.fromString("{{0, 5, 3}, {2, -3, 1}, {-9, 3, 4}}");
		LUResult result = LUDecomposition.decompose(m);
		assertEquals(-148.0, result.getDet(), 0.0D);
	}
	...
	public void testDet4() {
		Matrix m = Matrix.fromString("{{4, 3, 2, 1}, {1, 10, 3, 4}, {5, 3, 2, -4}, {4, 8, 7, 9}}");
		LUResult result = LUDecomposition.decompose(m);
		assertEquals(602.0D, result.getDet(), 0.0D);
	}
