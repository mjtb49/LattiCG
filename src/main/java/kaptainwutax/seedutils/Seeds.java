...
public class Seeds {
	...
	public static long setWeakRegionSeed(Rand rand, long worldSeed, int chunkX, int chunkZ) {
		int sX = chunkX >> 4;
		int sZ = chunkZ >> 4;
	...
	public static long setDecoratorSeed(Rand rand, long populationSeed, int genStep, int id) {
		long seed = populationSeed + genStep * 10000L + id;
		if(rand != null)rand.setSeed(seed, true);
		return seed;
	}

	public static long setCarverSeed(Rand rand, long worldSeed, int chunkX, int chunkZ) {
		if(rand == null)rand = new Rand(0L);
		rand.setSeed(worldSeed, true);
